...
bujpsi_filename = '../Ntuples/2018/Muon_Validation/JPsiSelection_combined_mc_BuJPsi_private_dnn_trackerMuonId.root'
bparking_filename = '../Ntuples/2018/Muon_Validation/JPsiSelection_combined_data_BScut_trackerMuonId_dnn.root'
...
validation_data = tree_bparking.pandas.df(['Muon_seg_comp_dnn','Muon_IsTrackerMuon','Muon_IsGlobalMuon','Muon_IsPFMuon','InvMass'])
validation_mc = tree_bujpsi.pandas.df(['Muon_seg_comp_dnn','Muon_IsTrackerMuon','Muon_IsGlobalMuon','Muon_IsPFMuon','InvMass'])
...
for ibin in xrange(len(dnn_bins)-1):
    jpsi_mass_categories['data']['dnn_bin_'+str(ibin+1)] = select_tracker_muons(validation_data[(validation_data['Muon_seg_comp_dnn']>dnn_bins[ibin])&
                                                                       (validation_data['Muon_seg_comp_dnn']<dnn_bins[ibin+1])])['InvMass'].to_numpy()
    jpsi_mass_categories['mc']['dnn_bin_'+str(ibin+1)] = select_tracker_muons(validation_mc[(validation_mc['Muon_seg_comp_dnn']>dnn_bins[ibin])&
...
for i in range(1, len(dnn_bins)):
    binstr = 'dnn_bin_'+str(i)
    fitter_ = jpsi_fitter()
    fitter_.fit_data(rds_data[binstr])
    fitter_.plot(rds_data[binstr], c, pad1, pad2, 'fits/jpsi_mumu_tracker_muon_data_dnn_'+binstr+'.png')
